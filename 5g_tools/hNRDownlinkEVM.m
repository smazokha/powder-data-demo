...
% Copyright 2019-2024 The MathWorks, Inc.
    ...
    pdcchDmrsEqSym = pdcchEqSym;                      % PDCCH DM-RS equalized symbols for each BWP
    pdcchDmrsRefSym = pdcchEqSym;                     % PDCCH DM-RS reference symbols for each BWP
    dmrsEqSym = eqSym;                                % PDSCH DM-RS equalized symbols for each BWP
    dmrsRefSym = refSym;                              % PDSCH DM-RS reference symbols for each BWP
    ptrsEqSym = eqSym;                                % PDSCH PT-RS equalized symbols for each BWP
    ptrsRefSym = refSym;                              % PDSCH PT-RS reference symbols for each BWP
    ...
    for bwpIdx = 1:numBWPs
        ...
        if pdschCfgLen
            ...
            [eqSlotGrids,refSlotGrids,...
                ~,eqSyms,refSyms] = hDecodeSlots(carrier,rxGrids,hest,...
            ...
            % basis
            eqSym(:,bwpIdx) = eqSyms.PXYCH;
            refSym(:,bwpIdx) = refSyms.PXYCH;
            dmrsEqSym(:,bwpIdx) = eqSyms.DMRS;
            dmrsRefSym(:,bwpIdx) = refSyms.DMRS;
            ptrsEqSym(:,bwpIdx) = eqSyms.PTRS;
            ptrsRefSym(:,bwpIdx) = refSyms.PTRS;
            ...
            if excludeDC
                ...
                d = find(refSym{1,bwpIdx} == 0);
                eqSym{1,bwpIdx}(d) = [];
                refSym{1,bwpIdx}(d) = [];

                dmrsEqSym{1,bwpIdx}(d) = [];
                dmrsRefSym{1,bwpIdx}(d) = [];
                ...
                if evm3GPP
                    eqSlotGrid(dcInd,:,:,:) = 0;
                    refSlotGrid(dcInd,:,:,:) = 0;
                    d = find(refSym{2,bwpIdx} == 0);
                    eqSym{2,bwpIdx}(d) = [];
                    refSym{2,bwpIdx}(d) = [];

                    dmrsEqSym{2,bwpIdx}(d) = [];
                    dmrsRefSym{2,bwpIdx}(d) = [];
                end
            end
        end
        ...
        for pdcchCfgIdx = 1:size(pdcchWaveCfg,2)
            ...
            if currentPdcchCfg.PDCCH.Enable
                ...
                [eqs,refs,~,eqSyms,refSyms] = hDecodeSlots(carrier,{pdcchRxGrid},{pdcchHest},...
                ...
                eqDmrs = eqs.DMRS;
                refDmrs = refs.DMRS;
                tmpEqSym = eqSyms.PXYCH;
                tmpRefSym = refSyms.PXYCH;
                tmpDmrsEqSym = eqSyms.DMRS;
                tmpDmrsRefSym = refSyms.DMRS;

                if isempty(pdcchEqSlotGrid) ||  sum(size(eq) - size(pdcchEqSlotGrid)) 
                   pdcchEqSlotGrid = zeros(size(eq));
                   pdcchRefSlotGrid = pdcchEqSlotGrid;
                   pdcchDmrsEqSlotGrid = pdcchEqSlotGrid;
                   pdcchDmrsRefSlotGrid = pdcchEqSlotGrid;
                end
                pdcchEqSlotGrid = pdcchEqSlotGrid + eq;
                pdcchRefSlotGrid = pdcchRefSlotGrid + ref;

                pdcchDmrsEqSlotGrid = pdcchDmrsEqSlotGrid + eqDmrs;
                pdcchDmrsRefSlotGrid = pdcchDmrsRefSlotGrid + refDmrs;

                pdcchEqSym{:,bwpIdx} = [pdcchEqSym{:,bwpIdx} ; tmpEqSym{:}];
                pdcchRefSym{:,bwpIdx} = [pdcchRefSym{:,bwpIdx} ; tmpRefSym{:}];

                pdcchDmrsEqSym{:,bwpIdx} = [pdcchDmrsEqSym{:,bwpIdx} ; tmpDmrsEqSym{:}];
                pdcchDmrsRefSym{:,bwpIdx} = [pdcchDmrsRefSym{:,bwpIdx} ; tmpDmrsRefSym{:}];
            end
        end
        if excludeDC
            ...
            if ~isempty(d)
                ...
                pdcchDmrsEqSym{1,bwpIdx}(d) = [];
                pdcchDmrsRefSym{1,bwpIdx}(d) = [];
            end
            if evm3GPP && ~isempty(d)
                ...
                pdcchDmrsEqSym{2,bwpIdx}(d) = [];
                pdcchDmrsRefSym{2,bwpIdx}(d) = [];
            end
        end
        ...
        if pdschCfgLen
            ...
            tmpDmrs = hEVM(carrier,dmrsEqSlotGrid,dmrsRefSlotGrid);
            tmpPtrs = hEVM(carrier,ptrsEqSlotGrid,ptrsRefSlotGrid);
            evm = tmp.EVM;
            evmDmrs = tmpDmrs.EVM;
            evmPtrs = tmpPtrs.EVM;
