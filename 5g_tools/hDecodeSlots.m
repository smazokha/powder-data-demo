function [eqSlotGrids,refSlotGrids,emissions,eqSyms,refSyms] = hDecodeSlots(carrier,rxGrids,hest,...
    ...
    %
    %   [EQSLOTGRIDS,REFSLOTGRIDS,~,EQSYMS,REFSYMS] =
    ...
    %   EQSYMS and REFSYMS have three fields, PXYCH, DMRS and PTRS. Each of
    %   these fields is an output cell array of IQ constellations(for
    %   EQSYMS) or reference IQ constellations (for REFSYMS) for low and
    %   high EVM window locations . It is of dimensions E-by-1. Each array
    %   element is of dimensions numIqSamples-by-L for low and high EVM
    %   window locations.It is of dimensions E-by-1.
    ...
    for slotIdx=slotRange
        ...
        [pxschRxLow,pxschHestLow,reInd] = nrExtractResources(channelDmrsInd,currentRxGridLow,currentHestLow);
        ...
        if nLayers >1
            tempGrid = nrResourceGrid(carrier,nLayers);
        ...
        if decodePdsch
            for idx = 1:length(pxschObj)
                ...
                if any(mod(tmpSlotIdx,period) == cfgArray(idx).PDSCH.SlotAllocation) && cfgArray(idx).PDSCH.EnablePTRS
                    ...
                    ptrsEqLow = [];
                    ptrsEqHigh = [];
                    [pdschEq, tmpPtrsEqLow] = pdschCPE(carrier,pxschObj{idx},channelIndices,ptrsIndices,ptrsSymbols,currentRxGridLow,currentHestLow);
                    eqGridLow = [eqGridLow; pdschEq];
                    ptrsEqLow = [ptrsEqLow; tmpPtrsEqLow];
                    if evm3GPP
                        [pdschEq, tmpPtrsEqHigh] = pdschCPE(carrier,pxschObj{idx},channelIndices,ptrsIndices,ptrsSymbols,currentRxGridHigh,currentHestHigh);
                        eqGridHigh = [eqGridHigh; pdschEq];
                        ptrsEqHigh = [ptrsEqHigh; tmpPtrsEqHigh];
                    end
                    break;
                end
            end
...
function [pdschEq,ptrsEq] = pdschCPE(carrier,pdsch,pdschIndices,ptrsIndices,ptrsSymbols,rxGrid,estChannelGrid)
%[PDSCHEQ,PTRSEQ] = pdschCPE(CARRIER,PDSCH,PDSCHINDICES,PTRSINDICES,PTRSSYMBOLS,RXGRID,ESTCHANNELGRID)
    % Slot-based Common phase error (CPE) estimation and correction for
